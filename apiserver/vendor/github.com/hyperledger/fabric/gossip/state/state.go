...
func NewGossipStateProvider(chainID string, services *ServicesMediator, ledger ledgerResources) GossipStateProvider {
	...
	maxBlock := viper.GetInt("peer.gossip.defMaxBlockDistance")
	logger.Debugf("peer.gossip.defMaxBlockDistance: %d", maxBlock)
	if maxBlock > 0 {
		s.defMaxBlockDistance = maxBlock
	}

	if viper.GetBool("peer.gossip.nonBlockingCommitMode") {
		s.blockingMode = false
	}
	logger.Debugf("peer.gossip.BlockingCommitMode: %v", s.blockingMode)
...
func (s *GossipStateProviderImpl) addPayload(payload *proto.Payload, blockingMode bool) error {
	...
	if !s.blockingMode && payload.SeqNum-height >= uint64(s.defMaxBlockDistance) {
		return errors.Errorf("Ledger height is at %d, cannot enqueue block with sequence of %d", height, payload.SeqNum)
	}

	for s.blockingMode && s.payloads.Size() > s.defMaxBlockDistance*2 {
